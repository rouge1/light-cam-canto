...
- **OSD-tainted block rows masked in BOTH `do_grid_calibration` AND `do_search`.** Channel 1 (the 640×360 substream BrightnessMonitor reads) carries OSD overlays in the top 30 px (timestamp + name + uptime) and bottom 30 px (`thingino` watermark). Their per-second character changes inject false delta into block rows `by=0` (blocks 0..19) and `by=11` (blocks 220..239). Without masking, the argmax can pick an OSD-edge block over the true TX block — observed 2026-05-09 with cam1 landing at `(34, 57)` (block 21, row 1, adjacent to OSD strip) when the real TX was at host-confirmed `(328, 114)` (block 70). 2026-05-10: the bootstrap-fallback `do_search` was independently observed picking block 34 (row 1) and writing `(477, 47)` into cam2's saved cal — its `SEARCH_HOLD_MS=3000` gate did NOT provide enough natural rejection. Both functions now zero `by=0` and `by=11` before argmax, mirroring `cal_procedure.py`'s `[:30, :]` / `[h-30:, :]`. `do_grid_calibration` also writes the full 240-block post-mask delta vector + best-block to `/run/grid-deltas.json` (atomic temp+rename), served via `/x/grid-deltas.cgi` → `CamStatusClient.get_grid_deltas()` → webui `/api/{cam}/grid-deltas` for runner-up debug over HTTP without SSH.
