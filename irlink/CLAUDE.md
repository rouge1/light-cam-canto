...
- **Adaptive rate: split-brain after probe-up ACK loss.** When a side probes up via `MSG_RATE_CHANGE`, the peer auto-ACKs at the OLD rate. If that ACK doesn't decode on the originator, the peer has switched to the new rate but the originator still thinks it's at the old rate. Neither side immediately notices. Split-brain recovery catches this after `2 × ack_timeout_ms` (~5 min at 160ms) by forcing **the FLOOR rung (160ms)**, not the slowest rung (200ms). Going to floor (instead of slowest) keeps recovered cams convergent with fresh-launched cams that lock at the floor — observed 2026-05-10: cam1 recovered to 200ms while cam2 launched a monocal at 160ms; they couldn't decode each other and the user manually had to restart cam1. With recovery → floor, both end up at 160ms regardless of which path. The recovery gate must check `last_valid_frame_ms` **only** — do NOT gate on `last_carrier_ms` as well, because a peer stuck at a mismatched rate still produces carrier (their TX is visible as activity) but never produces a valid decode, which IS the signal of split-brain. (Also: empirically 200ms is less reliable than 160ms for SYN/SYN_ACK round-trips at 18fps BrightnessMonitor cadence, so going to "slowest" wasn't even the most-reliable fallback — going to floor is.)
...
- **OSD-tainted block rows masked in `do_grid_calibration`, `do_search`, AND `grid_watchdog_thread`.** Channel 1 (the 640×360 substream BrightnessMonitor reads) carries OSD overlays in the top 30 px (timestamp + name + uptime) and bottom 30 px (`thingino` watermark). Their per-second character changes inject false delta into block rows `by=0` (blocks 0..19) and `by=11` (blocks 220..239). The watermark also visibly bleeds **upward into by=10** (blocks 200..219). Without masking, the argmax can pick an OSD-edge block over the true TX block. Observation log: (a) 2026-05-09 cam1 landed at `(34, 57)` block 21 row 1 adjacent to OSD strip when real TX was at host-confirmed `(328, 114)` block 70; (b) 2026-05-10 bootstrap-fallback `do_search` picked block 34 (row 1) and wrote `(477, 47)` into cam2's saved cal — its `SEARCH_HOLD_MS=3000` gate didn't reject; (c) 2026-05-10 watchdog fired block 220 (row 11) → wrote `(2, 347)` after rows 0/11 mask added to `do_grid_calibration` only; (d) 2026-05-10 even after watchdog mask added for rows 0/11, watchdog fired block 200 (row 10) → wrote `(17, 302)` because watermark bleed reached row 10. Final mask zeros `by=0`, `by=10`, AND `by=11` before argmax in all three sites, mirroring `cal_procedure.py`'s `[:30, :]` / `[h-30:, :]` (extended by one row at the bottom). `do_grid_calibration` also writes the full 240-block post-mask delta vector + best-block to `/run/grid-deltas.json` (atomic temp+rename), served via `/x/grid-deltas.cgi` → `CamStatusClient.get_grid_deltas()` → webui `/api/{cam}/grid-deltas` for runner-up debug over HTTP without SSH.
